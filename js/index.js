...
function Slider() {
    // 获取news区域
    this.news = document.querySelectorAll(".news")[0];
    // 获取轮播区域mainBox
    this.mainBox = document.querySelectorAll(".mainBox")[0];
    // 获取div 元素所在区域newListAll
    this.newListAll = this.mainBox.querySelectorAll(".newListAll")[0];
    // 获取每个列表newlist  a
    this.newlist = this.newListAll.children;
    // 取列表newlist的长度，即a的个数
    this.length = this.newlist.length
    // 获取newlist的单位宽度
    this.perWidth = this.newlist[0].offsetWidth;
    // 执行自动轮播
    this.autoPlay();
    // 清除定时器，不自动轮播
    this.clear();
...
Slider.prototype.move = function(){
    this.count++;
    // 对于轮播区域的运动的判断 
    if (this.count == this.length-4) {   //因为这个轮播图的范围是4个列表，每运动一个，都要保证后面的不为空
...
// 加15是因为css中设置每个列a有15px的右边距
    startMove(this.newListAll, { "left": -(this.perWidth + 15) * this.count});
...
Slider.prototype.addBtn = function(){
    ...
    // 获取左右按钮
    this.btnlr = this.btns.children;

    // 左箭头点击事件
    this.btnlr[0].onclick = () => {
        this.count -= 2;
        this.move();
        console.log("向左移动");
    }
    // 右键头点击事件
    this.btnlr[1].onclick = () => {
        this.move();
        console.log("向右移动");
    }
    }
